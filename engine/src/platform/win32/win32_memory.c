...
pub U64 memoryPageSizeGet(void) {
    SYSTEM_INFO info;
    GetSystemInfo(&info);
    return info.dwPageSize;
}

pub void* memoryReserve(U64 size) {
    size = roundUpToMultipleOf(size, gigabytes(1));
    void* pResult = VirtualAlloc(0, size, MEM_RESERVE, PAGE_NOACCESS);
    return pResult;
}

pub void memoryRelease(void* pMemory, U64 size) {
    VirtualFree(pMemory, 0, MEM_RELEASE);
}

pub void memoryCommit(void* pMemory, U64 size) {
    size = roundUpToMultipleOf(size, memoryPageSizeGet());
    VirtualAlloc(pMemory, size, MEM_COMMIT, PAGE_READWRITE);
}

pub void memoryDecommit(void* pMemory, U64 size) {
    VirtualFree(pMemory, size, MEM_DECOMMIT);
}

pub void memoryProtect(void* pMemory, U64 size, MemoryAccessFlags flags) {
    size = roundUpToMultipleOf(size, memoryPageSizeGet());
