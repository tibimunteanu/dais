...
ifeq ($(OS),Windows_NT)
	...
	EXTENSION := .dll
	COMPILER_FLAGS := -Wall -Werror -Wvla -Werror=vla -Wgnu-folding-constant -Wno-missing-braces -fdeclspec -Wstrict-prototypes -Wno-unused-function -Wno-unused-parameter -Wno-missing-field-initializers
...
# Make does not offer a recursive wildcard function, and Windows needs one, so here it is:
    ...
    ifeq ($(UNAME_S),Linux)
		...
		EXTENSION := .so
		COMPILER_FLAGS := -Wall -Werror -Wvla -Werror=vla -Wgnu-folding-constant -Wno-missing-braces -fdeclspec -Wstrict-prototypes -fPIC -Wno-unused-function -Wno-unused-parameter -Wno-missing-field-initializers
		INCLUDE_FLAGS := -I$(ASSEMBLY)/src $(ADDL_INC_FLAGS)
		LINKER_FLAGS := -shared -lxcb -lX11 -lX11-xcb -lxkbcommon -L/usr/X11R6/lib -L./$(BUILD_DIR) $(ADDL_LINK_FLAGS)
		# .c files
...
ifeq ($(BUILD_PLATFORM),windows)
	@clang $(OBJ_FILES) -o $(BUILD_DIR)\$(ASSEMBLY)$(EXTENSION) $(LINKER_FLAGS) -Xlinker /PDB:$(BUILD_DIR)\$(ASSEMBLY).pdb 
else
